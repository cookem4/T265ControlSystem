...
class PB_Control():
    ...
    def map_commands(self):
        ...
        for i in range (len(self.throttleMSP)):
            self.throttleMSP[i] = int(self.throttle[i] * (self.MAX_THROTTLE_MSP - self.MIN_THROTTLE_MSP) + self.MIN_THROTTLE_MSP) # This is mapped to an RC command in the range of 1100 to 2000.
            ...
            self.yawRateMSP[i] = int(-self.yawRate[i] * (1800 / math.pi) /2 + self.RPY_OFFSET) # This is half yawRate in decimal degrees per second. Stupid NOTE: MSP yaw rate interpretes the rate with a negative sign. 
##            print(self.yawRateMSP[i])
